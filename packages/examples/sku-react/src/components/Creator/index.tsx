...
const SkuCreator: FC<Props> = (props) => {
  ...
  skus?.forEach((sku) => {
    sku.properties?.forEach((prop, i) => {
      if (prop.name && prop.value.text) {
        const currentProp = defaultProperties.find((p) => p.name === prop.name);
        if (currentProp) {
          !currentProp.values?.includes(prop.value.text) &&
            currentProp.values?.push(prop.value.text);
        } else {
          defaultProperties.push({
            name: prop.name,
            key: uuid(),
            values: [prop.value.text],
          });
        }
      }
    });
  ...
  useEffect(() => {
    ...
    const properties: { name: string; value: { text: string } }[][] = [];
    ...
    _rows.forEach((r, rindex) => {
      r.properties?.forEach((p, pindex) => {
        ...
        if (rowCount[pindex] !== p.value.text) {
          colSpanArray[p.name].push(rindex - 1);
          rowCount[pindex] = p.value.text;
    ...
    _columns.forEach((c) => {
      c.render = (value, _, index) => {
        ...
        obj.props.rowSpan = 0;
        colSpanArray[c.title as string]?.forEach((i: any, cindex: number) => {
